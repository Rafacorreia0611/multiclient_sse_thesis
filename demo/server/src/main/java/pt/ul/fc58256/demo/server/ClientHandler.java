...
public class ClientHandler implements Runnable {
    ...
    private void handleSearch() {
        ...
        Map<EncryptedUpdateTuple, SecretKey> searchResults = sseServer.searchQuery(searchToken);
        send(searchResults);
    }
    ...
    private void handleUpdate() {
        ...
        while (true) {
            ...
            SecretKey updateTupleKey = readExpected(SecretKey.class);
            if (updateToken == null) {
                throw new RuntimeException("Client disconnected before sending update token");
            }
            try {
                sseServer.updateQuery(updateToken, updateTupleKey);
